...
// Student connection
router.post('/sshLogin', async (req, res) => {
  try {
...
router.post('/dbLogin', async (req, res) => {
  try {
    ...
    console.log('Connexion à la DB établie avec succès');
    res.sendStatus(200);
  } catch (error) {
    console.error('Erreur lors de la connexion à la DB de l\'étudiant', error);
    res.status(500).json({ error: 'Erreur lors de la connexion à la DB de l\'étudiant' });
  }
});
